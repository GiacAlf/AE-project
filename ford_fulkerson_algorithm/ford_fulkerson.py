...
The time complexity of this implementation is: O(n^3*m^2), because of the nested for loop
 with a O(n*m^2) complexity for nx.maximum_flow function calls.
"""
...
def ford_fulkerson_min_cut(G):
    ...
    for i in range(len(nodes)):
        for j in range(i + 1, len(nodes)):
            # uses maximum flow algorithm to find maximum flow (Edmond-Karp)
